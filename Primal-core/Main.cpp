...
struct Character
{
	...
	primal::maths::vec2 Size;    // Size of glyph
	primal::maths::vec2 Bearing;  // Offset from baseline to left/top of glyph
...
void RenderText(primal::graphics::Shader &shader, std::string text, GLfloat x, GLfloat y, GLfloat scale, primal::maths::vec3 color);
...
int main()
{
	...
	primal::graphics::Shader shader("./res/shader/basic-test.vert", "./res/shader/basic-test.frag");
	primal::maths::mat4 projection = primal::maths::mat4::orthographic(0.0f, static_cast<GLfloat>(WIDTH), 0.0f, static_cast<GLfloat>(HEIGHT), -1.0f, 1.0f);
	shader.enable();
	...
	for (GLubyte c = 0; c < 128; c++)
	{
		...
		Character character = {
			texture,
			primal::maths::vec2(face->glyph->bitmap.width, face->glyph->bitmap.rows),
			primal::maths::vec2(face->glyph->bitmap_left, face->glyph->bitmap_top),
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		RenderText(shader, "This is sample text", 25.0f, 25.0f, 1.0f, primal::maths::vec3(0.5, 0.8f, 0.2f));
		RenderText(shader, "(C) LearnOpenGL.com", 540.0f, 570.0f, 0.5f, primal::maths::vec3(0.3, 0.7f, 0.9f));
